...
interface UrlContentDetailsProps {
  data: UrlDetail | null;
  isLoading?: boolean;
}

function UrlContentDetails({ data, isLoading }: UrlContentDetailsProps) {
  if (isLoading) {
    return (
  ...
  return (
    <Card className="h-full flex flex-col min-h-0">
      <div className="flex-shrink-0 flex justify-between items-start mb-4 gap-4">
        <h3 className="font-semibold break-all min-w-0">{data.url}</h3>
      ...
      <div className={`flex-1 overflow-auto border-t border-gray-100 pt-3 min-h-0`}>
        <pre className="m-0 whitespace-pre text-sm text-gray-800 min-w-max">
          {data.content || <span className="text-gray-500">No content available for this URL.</span>}
        </pre>
      </div>
    </Card>
  );
...
export default UrlContentDetails;